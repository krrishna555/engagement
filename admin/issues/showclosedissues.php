...
<a href="fetchissues.php"><button for="submit_btn" type="submit" id="submit" name="submit" value="SUBMIT" class="btn btn-primary pull-right">Show open issues</button></a></br></br>
<?php
...
where issue_solved=1 AND issue_categories.is_active=1 ORDER BY created_at DESC"; 
$result = mysqli_query($conn, $query) or die ( mysqli_error());
$upload_url = "https://".$_SERVER['HTTP_HOST']."/uploads/";
echo "<table id='example' class='table table-striped' style='width:100%' >";

echo "<tr> <th>image</th><th>Issue Type</th><th>location</th><th>Issue Created Date</th></tr>";
if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
		$file = $upload_url.$row['image_url'];?>
<tr class="clickable-row" data-href="issueinfoclosed.php?locationid=<?php echo $row['id'];?>"><?php
if(!empty($row['image_url']) || (file_exists($file)==1))
	//if(file_exists($upload_url))
{?>
  <td style="width:25%"><img height="80" width="130" src="<?php echo $upload_url.$row['image_url'];?>"> </td>  <?php
}
else
{ ?>
 <td style="width:25%"><img height="80" width="130" src="<?php echo $upload_url.'noimage.png'?>"> </td> <?php
} ?>
