...
<div class="main col-md-10 col-xs-10 col-lg-12" >
<a href="pointperson.php"><button for="submit_btn" type="submit" id="submit" name="submit" value="SUBMIT" class="btn btn-primary pull-right">Show open issues</button></a></br></br>
<?php

include("conn.php");
 $user_id = $_SESSION['id'];
$queryissues = "SELECT i.id,i.location,i.image_url,i.created_at,it.issue_typename,ic.category_name from issues i INNER JOIN issue_types it on it.id=i.type_id INNER JOIN issue_categories ic on ic.id = it.category_id where it.user_id=$user_id and issue_solved=1 ORDER BY created_at DESC";
...
if (mysqli_num_rows($resultiss) > 0) {
	...
	<tr class="clickable-row" data-href="issueinfoclosedperson.php?locationid=<?php echo $row['id'];?>"><?php
